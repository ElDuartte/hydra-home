...
const SCHEMA = {
    ...
    intervals: {
        default: { weatherMs: 1800000, jellyfinMs: 5000, glancesDefaultMs: 3000, cpuMs: 2000, memMs: 1000, diskMs: 3600000 },
        validate: (val) => val && typeof val === 'object',
    },
    thresholds: {
        default: { cpuWarning: 80, ramWarning: 85, diskWarning: 85, tempWarning: 70, tempCritical: 85 },
        validate: (val) => val && typeof val === 'object',
    },
};
    ...
    _validate(raw) {
        ...
        Object.entries(SCHEMA).forEach(([key, schema]) => {
            ...
            if (value === undefined) {
                ...
                // For intervals and thresholds, merge with defaults to ensure all keys exist
                if (key === 'intervals' || key === 'thresholds') {
